...
def parse_arguments(argv):
    ...
    analysis.add_argument('--cc',
                          dest='concurrent',
                          help='Enable concurrent fixpoint iteration',
                          action='store_true',
                          default=False)
    analysis.add_argument('-nt',
                          dest='numthreads',
                          metavar='',
                          help='Number of threads',
                          type=int,
                          default=4)
    analysis.add_argument('-cs',
                          dest='contextsensitivity',
                          metavar='',
                          help='Context sensitivity',
                          type=int,
                          default=0)
...
def ikos_analyzer(db_path, pp_path, opt):
    ...
    if opt.concurrent:
        cmd.append('-cc')
        if opt.numthreads is not None:
            cmd.append('-nt=%d' % opt.numthreads)
    if opt.contextsensitivity is not None:
        cmd.append('-cs=%d' % opt.contextsensitivity)
    if opt.no_checks:
...
def main(argv):
    ...
    opt.output_db = "output-" + os.path.basename(opt.file) + "-" + str(random.randint(1, 99)) + ".db"
