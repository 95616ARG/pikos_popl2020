...
static llvm::cl::opt< int > ContextSensitivity(
    "cs",
    llvm::cl::desc("Context Sensitivity"),
    llvm::cl::init(0),
    llvm::cl::cat(AnalysisCategory));

static llvm::cl::opt< int > NumThreads(
    "nt",
    llvm::cl::desc("Number of threads for fixpoint iteration"),
    llvm::cl::init(8),
    llvm::cl::cat(AnalysisCategory));

static llvm::cl::opt< bool > Concurrent(
    "cc",
    llvm::cl::desc("Enable the concurrent fixpoint iteration"),
    llvm::cl::cat(AnalysisCategory));
...
static llvm::cl::opt< bool > NoChecks(
    "no-checks",
    llvm::cl::desc("Disable the checks"),
    llvm::cl::cat(AnalysisCategory));
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    if (OutputFilename == "output.db") {
      OutputFilename = "output-" + path.filename().native() + ".db";
    }
    ...
    if (Procedural == analyzer::Procedural::Interprocedural) {
      if (Concurrent) {
        if (NumThreads >= 100) {
          analyzer::cc::InterproceduralValueComparer comparer(ctx);
          analyzer::log::info("Running interprocedural value comparer");
          int ret;
          if ((ret = comparer.run()) != 0) {
            std::cerr << "\nUNMATCH!!(" << ret << ")\n";
          }
          return ret;
        }

        analyzer::cc::InterproceduralValueAnalysis analysis(ctx);
        analyzer::log::info("Running concurrent interprocedural value analysis");
        analyzer::ScopeTimerDatabase t(output_db.times,
                                       "ikos-analyzer.value-analysis-concurrent");
        ikos_assert_msg(NumThreads > 0, "Invalid number of threads");
        analysis.run();
      } else {
      ...
      if (Concurrent) {
        analyzer::cc::IntraproceduralValueAnalysis analysis(ctx);
        analyzer::log::info("Running concurrent intraprocedural value analysis");
        analyzer::ScopeTimerDatabase t(output_db.times,
                                       "ikos-analyzer.value-analysis-concurrent");
        ikos_assert_msg(NumThreads > 0, "Invalid number of threads");
        analysis.run();
