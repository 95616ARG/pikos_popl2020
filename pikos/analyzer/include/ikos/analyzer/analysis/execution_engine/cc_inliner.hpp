...
private:
  ...
  using CalleeMap =
      tbb::concurrent_unordered_map< ar::Function*,
                                     std::unique_ptr< FunctionAnalyzer > >;

  /// \brief Map from call statement to CalleeMap
  using CallMap = tbb::concurrent_unordered_map< ar::CallBase*, CalleeMap >;
...
public:
  ...
  void set_return_stmt(ar::ReturnValue* return_stmt) { this->_return_stmt = return_stmt; }
...
private:
  ...
  struct CalleeAnalysis {
    std::unique_ptr< FunctionAnalyzer > callee_analyzer;
    InlineCallExecutionEngineT* caller_inliner;
    NumericalExecutionEngineT caller_engine;
    ar::CallBase* call;
    ar::Function* callee;
    bool converged; // True if callee_analyzer converged

    CalleeAnalysis(InlineCallExecutionEngineT* caller_inliner_)
      : callee_analyzer(nullptr)
      , caller_inliner(caller_inliner_) 
      , caller_engine(caller_inliner->_engine.fork())
      , converged(false) {}
  };

  struct JoinBody {
    // Reference to all the callee analysis.
    std::vector<CalleeAnalysis>& input;
    AbstractDomain value; 

    JoinBody(std::vector<CalleeAnalysis>& callee_analysis_)
      : input(callee_analysis_)
      , value(AbstractDomain::bottom()) {}
    JoinBody(JoinBody& body, tbb::split)
      : input(body.input)
      , value(AbstractDomain::bottom()) {}

    void operator()(const tbb::blocked_range<size_t>& r) {
      for (auto ind = r.begin(); ind != r.end(); ind++) {
        CalleeAnalysis& ca = input[ind];
        // Run analysis on callee, if needed.
        if (!ca.converged) {
          ca.callee_analyzer->run(ca.caller_engine.inv());
        }
        // Return statement in the callee, or null
        ar::ReturnValue* return_stmt = ca.callee_analyzer->return_stmt();
        ca.caller_engine.set_inv(ca.callee_analyzer->exit_invariant());
        // Merge exceptions in caught_exceptions, in case it's an invoke
        ca.caller_engine.inv().merge_propagated_in_caught_exceptions();
        if (!ca.caller_engine.inv().is_normal_flow_bottom()) {
          ca.caller_engine.match_up(ca.call, return_stmt);
        }
        // Reduce
        value.join_with(std::move(ca.caller_engine.inv()));
      }
   }

   void join(JoinBody& rhs) { value.join_with(rhs.value); }
  };
  ...
  void exec(ar::CallBase* call) {
    ...
    // In too deep, give up.
    if (this->_caller.has_reached_context_sensitivity_limit()) {
    ...
    // For each callee
    // Use imperative style instead of functional style to minize the copy.

    std::vector< CalleeAnalysis > analysis;
    ...
    for (MemoryLocation* mem : callees) {
      ...
      analysis.emplace_back(this);
      CalleeAnalysis& callee_analysis = analysis.back();
      callee_analysis.call = call;
      callee_analysis.callee = callee;

      // Do not propagate exceptions from the caller to the callee
      callee_analysis.caller_engine.inv().ignore_exceptions();

      // Assign parameters
      callee_analysis.caller_engine.match_down(call, callee);
      ...
      if (this->_convergence_achieved && _ctx.opts.use_fixpoint_cache) {
        ...
        callee_analysis.converged = true; // No need to run engine.
      } else {
        ...
        callee_analysis.converged = false;
      }

      callee_analysis.callee_analyzer = std::move(callee_analyzer);
    }
    ...
    JoinBody joinBody(analysis);
    if (analysis.size() == 1) {
      joinBody(tbb::blocked_range<size_t>(0, analysis.size()));
    } else {
      tbb::parallel_reduce(tbb::blocked_range<size_t>(0, analysis.size()), joinBody, affinity);
    }

    for (auto& ca : analysis) {
      if (this->_check_callees) {
        // Run the checks on the callee
        ca.callee_analyzer->run_checks();
      }

      if (_ctx.opts.use_fixpoint_cache) {
        // Save the fix-point for later
        _caller._call_exec_engine._calls_cache[ca.call][ca.callee] = std::move(ca.callee_analyzer);
      } else {
        // Delete the callee fix-point
        ca.callee_analyzer.reset();
      }
    }

    post.join_with(std::move(joinBody.value));
    this->_engine.set_inv(std::move(post));
  }
